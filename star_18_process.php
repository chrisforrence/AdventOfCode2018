...
class Board {
    private $marbles = [0];
    public function clockwise($number) {
        for ($i = 0; $i > $number; $i--) {
           array_unshift($this->marbles, array_pop($this->marbles));
        }
    }

    public function counterclockwise($number) {
        for ($i = 0; $i < $number; $i++) {
            array_push($this->marbles, array_shift($this->marbles));
        }
    }
